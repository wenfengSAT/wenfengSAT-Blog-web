...
import com.blog.model.BlackIp;
import com.blog.service.BlackIpService;
...
public class BlackIpControllerAdmin {
	...
	@ResponseBody
	public Map<String, Object> selectLikeBlackIpListByPage(BlackIp blackIp,
		...
		if (!StringUtils.isEmpty(blackIp.getIp())) {
			map.put("ip", blackIp.getIp());
		}
		if (!StringUtils.isEmpty(blackIp.getCity())) {
			map.put("city", blackIp.getCity());
		}
		if (!StringUtils.isEmpty(blackIp.getPlatformtype())) {
			map.put("platformType", blackIp.getPlatformtype());
		}
		if (!StringUtils.isEmpty(blackIp.getBrowsertype())) {
